...
helper.checkPeriod = () => {
    ...
    let yesterdayLastPeriod = dateHelper.getYesterdayD(dateHelper.now()) + (now.getHours() == 0 && now.getMinutes() < 2 ? '1839' : '1840');
...
helper.setLostPeriod = (lPeriod, callback) => {
    ...
    if (lPeriod) {
        ...
        dbHelper.setFixLosePeriodRecord('c3', lPeriod);
...
helper.setLostPeriodRecord = (lPeriod) => {
    ...
    if (lPeriod) {
        dbHelper.setLosePeriod('c3', lPeriod);
