...
helper.setRc = (where, period) => {
    dbHelper.getRc(where, (item) => {

        if (!item)
            redisHelper.setLotteryWaitRc(period.lastNo);

        // 開獎
        redisHelper.lotteryBingoRc((bingoResult) => {
            ...
            // 是否還有待開獎期號
            redisHelper.getLotteryWaitRc((lotteryWaitRcResult) => {

                let lotteryWaitRcList = lotteryWaitRcResult.split('|');
                let url;
                let param;
                    ...
                    if (body) {
                        ...
                        try {
                            ...
                            if (json.result.data.preDrawIssue && json.result.data.preDrawCode && json.result.data.preDrawTime) {
                                ...
                                let index = lotteryWaitRcList.indexOf(param.no.toString());

                                if (index >= 0)
                                    redisHelper.setLotteryRc(param);
                            }
                        } catch (err) {
                            console.log('Rc:168 開獎網 ' + err);
                        }
                    }
                });
...
helper.setLostPeriodRecord = (lPeriod) => {
    //將遺漏的期號寫入資料表
    if (lPeriod) {
        dbHelper.setLosePeriod('rc', lPeriod);
    }
}
...
helper.setLostPeriod = (lPeriod, callback) => {
    if (lPeriod) {
        ...
        helper.setRcByPeriod(where, period, periodDay, (status) => {
            callback(status);
        });
...
helper.setRcByPeriod = (where, period, adDate, cb) => {
    dbHelper.getRc(where, (item) => {
