...
helper.getLotteryPeriod = (lotteryTime, timestamp) => {
    ...
    let startOfTodayTS = parseInt(dateHelper.getStartOfTodayTS(ts));
    let todayD = dateHelper.getTodayD(ts);
    let yesterdayD = dateHelper.getYesterdayD(ts);
    let tomorrowD = dateHelper.getTomorrowD(ts);

    if (ts < startOfTodayTS + 14640) {
        //(如果現在時間小於4:04AM，則開始日期往後一天)
        startOfTodayTS = parseInt(dateHelper.getStartOfTodayTS(ts - 86400));
        todayD = dateHelper.getTodayD(ts - 86400);
        yesterdayD = dateHelper.getYesterdayD(ts - 86400);
        tomorrowD = dateHelper.getTomorrowD(ts - 86400);
    }
