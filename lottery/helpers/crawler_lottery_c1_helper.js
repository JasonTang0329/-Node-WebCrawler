...
helper.checkPeriod = () => {
    ...
    let yesterdayLastPeriod = dateHelper.getYesterdayD(dateHelper.now()) + (now.getHours() == 0 && now.getMinutes() < 2 ? '0919' : '0920');
...
helper.setLostPeriod = (lPeriod, callback) => {
    ...
    if (lPeriod) {
        ...
        dbHelper.setFixLosePeriodRecord('c1', lPeriod);
...
helper.setLostPeriodRecord = (lPeriod) => {
    ...
    if (lPeriod) {
        ...
        dbHelper.setLosePeriod('c1', lPeriod);
